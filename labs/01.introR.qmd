...
You will also have a standard white background; but you can chose specific [themes](https://quarto.org/docs/output-formats/html-themes.html).
...
-   Toggling between the R Console, and the *Terminal* (only for shell commands, do not use it unless instructed to do so).
...
<span style="color: red;">This should be on your personal computer or if on a local machine, I suggest using the directory M:\ to store the folder, it can be accessed from every computer.</span>

Then, in R Studio
- on Windows by clicking RStudio\>Tools\>Global Options\>General.. 
- on Mac OS by clicking RStudio\>Preferences\>Appearance or Pane Layout...
...
Most of the time you just need to specify the fil.                                             |
...
Before starting your analysis, it is critical to know the type of scale used to measure your outcome variable: is it categorical or scale (continuous)? Here we will start off by exploring a scale (continuous) which can then turn into a categorical variable (e.g. top earners: yes or no). We explore the income distribution in the UK by first looking at the low and high end of the distribution ie. What sorts of people have high (or low) incomes?
...
Examine the frequency distribution of the variable you have just created. Both variables should have the same number of missing cases, unless:
 ...
 - You forgot to allocate a newvalue to one of the old variable categories, in which case the new variable will have more missing cases than the old variable.
...
Finally, investigate correlations between income and other variables of interest to explain variation.

```{r, warning=FALSE, message=FALSE}
# Correlation between income and other variables (e.g., age, education level)
# cor(frs_independent$income_gross, frs_independent$education_level, use = "complete.obs")
```