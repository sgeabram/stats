...
This section introduces you to using R for manipulating tabular data. Please read through it carefully and pay attention to how ideas about manipulating data are translated into code. For this part, you can read directly from the course website, although ***it is recommended you follow the section interactively by running the code on your own.***

Once you have read through, jump on the [Do-It-Yourself]() section, which will provide you with a challenge that you should complete on your own.

## Data wrangling



In this session, you will use a few real world datasets and learn how to process them in R so they can be transformed and manipulated, if necessary, and analyzed. For this, we will introduce some of the fundamental tools of data analysis and statistics. In this notebook, we discuss several patterns to clean and structure data properly, including tidying, subsetting, and aggregating. An additional extension presents more advanced tricks to manipulate tabular data.
...
## Datasets
A dataset is a structured collection of data points that represent various measurements or observations, often organized in a tabular format with rows and columns. For human geographers, a dataset might contain information about different locations, such as neighborhoods or cities, with each row representing a unique location and each column detailing characteristics like population density, average income, or access to public transport.

Datasets provide the essential building blocks for statistical analysis, enabling geographers to explore relationships, identify patterns, and draw conclusions about spatial phenomena and social dynamics in various locations.
...
-   The argument row.names is not strictly necessary but allows us to choose one of the columns as the index of the table. 

-   We are using `read.csv` because the file we want to read is in the csv format. However, many more formats can be read into an R environment.
